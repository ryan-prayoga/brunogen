...
import { writeArtifacts } from "../src/core/pipeline";
import { fixturePath, repoPath } from "./helpers";

describe("Express AST demo snapshots", () => {
  it("matches the checked-in happy path output", async () => {
    const sandboxRoot = await fs.mkdtemp(path.join(os.tmpdir(), "brunogen-express-ast-demo-"));
    ...
    try {
      ...
      const normalized = await scanExpressProjectAst(
        workingRoot,
        "acme/express-demo",
      ...
      const artifacts = {
        normalized,
        openApi: buildOpenApi(normalized, config),
      ...
      const actualBrunoRequest = normalizeBruSnapshot(await fs.readFile(
        path.join(generatedRoot, "bruno", "api", "createuser.bru"),
        "utf8",
      ));
      const expectedBrunoRequest = normalizeBruSnapshot(await readDemoSnapshot("bruno", "api", "createuser.bru"));
...
function normalizeBruSnapshot(content: string): string {
  return normalizeSnapshot(content).replace(/^(\s*seq:\s*)\d+$/m, "$1<auto>");
}
