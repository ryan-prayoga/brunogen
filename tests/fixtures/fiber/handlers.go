...
func listWidgets(c *fiber.Ctx) error {
	...
	return c.Status(fiber.StatusOK).JSON(fiber.Map{"data": []WidgetResponse{{Name: "Widget", Page: pageInt}}})
}
...
func createWidget(c *fiber.Ctx) error {
	...
	return c.Status(fiber.StatusCreated).JSON(fiber.Map{"message": "widget created", "data": WidgetResponse{Name: req.Name, Page: req.Page, Token: token}})
}
