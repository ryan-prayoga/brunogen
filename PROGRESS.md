...
- [x] Added default object export, barrel re-export, `export *`, inline `require()`, `express.Router()`, and repeated router mount coverage
- [x] Added statically computed Express AST mount and route path coverage for string constants, concatenation, and template literals
- [x] Added local Express AST router factory mount coverage for statically returned routers
- [x] Added Express AST dynamic import mount and custom local router abstraction coverage

## Next Steps

- [ ] Keep the regex-based Express scanner as default until AST output parity is broad enough for real projects
- [ ] Expand Express AST fixtures around non-static dynamic imports and higher-order custom router abstractions before making it the default path
